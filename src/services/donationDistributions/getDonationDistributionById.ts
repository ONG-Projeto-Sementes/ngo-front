...
export async function getDonationDistributionById(id: string): Promise<DonationDistribution> {
  try {
    const response = await apiClient.get<{ message: string; data: DonationDistribution }>(`/distributions/${id}`);
...
export default getDonationDistributionById;
